...
#include "aprilgrid/apriltag2/Tag36h11.h"
#include "aprilgrid/apriltag2/TagDetector.h"

namespace calibration_utils {

struct ApriltagDetectorData {
  ApriltagDetectorData(int startId, int numTags)
      : doSubpixRefinement(true), maxSubpixDisplacement(0),
        minTagsForValidObs(4), minBorderDistance(4.0), blackTagBorder(2),
        _tagCodes(AprilTags::tagCodes36h11), _startId(startId),
        _numTags(numTags) {
    _tagDetector =
  ...
  double maxSubpixDisplacement; //!< maximum displacement for subpixel
                                //!< refinement. If 0, only base it on tag size.
  ...
  int _startId; //!< starting tag id in the grid
...
ApriltagDetector::ApriltagDetector(int numTags) {
  data = new ApriltagDetectorData(0, numTags);
}

ApriltagDetector::ApriltagDetector(int startId, int numTags) {
  data = new ApriltagDetectorData(startId, numTags);
}

ApriltagDetector::~ApriltagDetector() { delete data; }

void ApriltagDetector::detectTags(
    const cv::Mat &img_raw, std::vector<Eigen::Vector2d> &corners,
  ...
  if (img_raw.channels() != 1) {
    if (img_raw.channels() == 3 || img_raw.channels() == 4) {
      cv::cvtColor(img_raw, image, cv::COLOR_BGR2GRAY);
    } else {
      std::cout << "error image type!" << std::endl;
      std::exit(0);
    }
  } else {
    img_raw.copyTo(image);
  }
  ...
  for (iter = detections.begin(); iter != detections.end();) {
    ...
    // detection)
    if (iter->id < data->_startId ||
        iter->id >= (data->_startId + data->_numTags))
      remove |= true;
  ...
  if (detections.size() < data->minTagsForValidObs)
    return;
  ...
  for (unsigned int i = 0; i < detections.size(); i++) {
    ...
    int gridIndex = tagId - data->_startId; // map to 0..numTags-1
