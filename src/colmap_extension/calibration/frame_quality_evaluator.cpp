...
bool FrameQualityEvaluator::isFrameAcceptable(
    const cv::Mat &image, const std::vector<cv::Point2f> &corners) {
  initialize(image);

  // if (!isSharp(image)) return false;
  if (!hasEnoughCorners(corners))
    return false;

  // if (!isDistributionReasonable(corners, img_size)) return false;

  std::vector<float> hist_current =
      local_coverage_.getHistogramOfCorners(corners);

  if (!isNewView(hist_current))
    return false;
...
bool FrameQualityEvaluator::hasEnoughCorners(
    const std::vector<cv::Point2f> &corners) const {
  return corners.size() >= min_corners_num_;
}

bool FrameQualityEvaluator::isNewView(
    const std::vector<float> &hist_current) const {
  if (saved_histograms_.empty())
    return true;
...
std::vector<float> FrameQualityEvaluator::DGrid::getHistogramOfCorners(
    const std::vector<cv::Point2f> &corners) const {
...
void FrameQualityEvaluator::DGrid::add(
...
void FrameQualityEvaluator::DGrid::initialize(const cv::Mat &img,
                                              int window_num) {
  if (!is_initial) {
    is_initial = true;
    ...
    for (int r = 0; r < rows; ++r) {
      int y = r * cell_height;
      cv::line(viz_mat, cv::Point(0, y), cv::Point(img.cols, y),
               cv::Scalar(0, 0, 255), 1);
    }

    for (int c = 0; c < cols; ++c) {
      int x = c * cell_width;
      cv::line(viz_mat, cv::Point(x, 0), cv::Point(x, img.rows),
               cv::Scalar(0, 0, 255), 1);
    }
  }
}
